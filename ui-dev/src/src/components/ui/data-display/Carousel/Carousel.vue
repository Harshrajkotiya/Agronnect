...
import type { ChatMessages, IAttachment, IConversation } from "@src/types";
...
const attachments = computed(() => {
  let attachments = [];
  if (props.mediaGroup && props.mediaGroup != undefined) {
    for (let index = 0; index < props.mediaGroup.length; index++) {
      let message = props.mediaGroup[index];
      if (message.media_files) {
        const isVideoOrImage =
          message.media_files.type !== undefined &&
          ["video", "image"].includes(message.media_files.type);
        const isWebp =
          message.media_files.name &&
          message.media_files.name.endsWith(".webp");
  ...
  if (props.message && props.message?.media_files != undefined) {
    attachments.push({
...
const startingIndex = computed(() => {
  ...
  attachments.value.forEach((value, index) => {
    ...
    if (value.id === props.startingId) {
     ...
     else if (index === props.startingId - 1) {
        startingIndex = props.startingId - 1;
    }
  }
  });
...
<template>
  <div
    :class="store.isIOS ? 'absolute z-10' : 'relative z-10'"
            ...
            :is-image="
              Boolean(
                selectedAttachment?.type === 'image' ||
                  selectedAttachment?.name?.endsWith('.webp'),
              )
            "
          ...
          <div class="h-full flex justify-between">
            ...
            >
              ...
              <VideoPlayer
                ...
                :url="selectedAttachment?.path"
                :thumbnail="selectedAttachment?.thumb_path as string"
                :key="selectedAttachment?.id"
              />
            </div>
