  ...
  <div class="flex flex-col">
    ...
    <div class="flex justify-start">
      <label v-if="props.label" :for="props.id" class="mb-3">
        <span
          class="w-13 text-black text-sm font-semibold leading-4 tracking-[0.16px]"
        >
          {{ props.label }}
        </span>
      </label>
    ...
    <label
      ...
      :for="props.id"
      tabindex="0"
      class="cursor-pointer w-full h-[100px] border border-dashed rounded-sm p-5 border-gray-200 flex justify-center items-center hover:bg-opacity-0 active:bg-opacity-0 focus:bg-opacity-0 outline-none focus:outline-none duration-500 transition-all"
      :class="{
        'bg-opacity-0': active,
        'bg-gray-50':
          !active,
